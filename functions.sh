...
function do_dump() {
  ...
  # if we asked to do by schema, then we need to get a list of all of the databases, take each, and then tar and zip them
    ...
    if [[ -z "$DB_NAMES" ]]; then
      DB_NAMES=$(psql -l | awk '$1 !~ /[+(|:]|Name|List|template|postgres/ {print $1}')
      [ $? -ne 0 ] && return 1
    fi
    for onedb in $DB_NAMES; do
      echo "Fazendo dump de $onedb"

      if [[  -f ${workdir}/globals.sql ]]; then
        rm ${workdir}/globals.sql
        echo "Dumping users and Perms"
        pg_dumpall  --globals-only -f ${workdir}/globals.sql
      else
        echo "Dumping users and Perms"
        pg_dumpall  --globals-only -f ${workdir}/globals.sql
      fi 

      pg_dump -Fc -f $PGSQL_OPTS $workdir/backup_${now}.sql ${onedb}
      
      [ $? -ne 0 ] && return 1
    done

  echo "Compressing data from $workdir to $TMPDIR/$SOURCE"
